 ...
 *  ----------------------------------------------------------------------------
 *  File:       Rule_ConditionalAssign.cs
...
namespace Ewav.Web.EpiDashboard.Rules
{
    ...
    public class Rule_ConditionalAssign : DataAssignmentRule
    {
        ...
        public override void SetupRule(DataTable table)
        {
            ...
            string s = DataFilters.GenerateDataFilterString();

            try
            {
                dvAssign = new DataView(table, s, string.Empty, DataViewRowState.CurrentRows);
            }
            catch (Exception)
            {
