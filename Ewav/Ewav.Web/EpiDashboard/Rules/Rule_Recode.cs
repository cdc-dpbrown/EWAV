...
namespace Ewav.Web.EpiDashboard.Rules
{
    ...
    /// <summary>
    /// A class designed to recode data from a form fields to a new, temporary user-defined fields based on a rule defined by the user
    /// </summary>
    public class Rule_Recode : DataAssignmentRule
    {
        ...
        /// <summary>
        /// Gets the name of the sourceObject column from which to read values
        /// </summary>
        ...
        /// <summary>
        /// Gets the type of the sourceObject column from which to read values
        /// </summary>
        ...
        public override System.Xml.XmlNode Serialize(System.Xml.XmlDocument doc)
        {
            ...
            foreach (DataRow row in recodeInputTable.Rows)
            {
                ...
                foreach (object obj in objects)
                {
                    ...
                    xmlString = xmlString + obj.ToString().Replace("<", "&lt;").Replace(">", "&gt;");
                    xmlString = xmlString + "</recodeTableData>";
                }
                xmlString = xmlString + "</recodeTableRow>";
            }
                    ...
                    foreach (XmlElement recodeRow in child.ChildNodes)
                    {
                        if (recodeRow.Name.ToLower().Equals("recodetablerow"))
                        {
                            ...
                            foreach (XmlElement recodeCell in recodeRow.ChildNodes)
                            {
                                ...
                                {
                                    itemArray[count] = recodeCell.InnerText;
                                    count++;
                                }
                            }
                ...
                if (!string.IsNullOrEmpty(value))
                {
                    ...
                    if (outerSuccess)
                    {
                        foreach (DataRow recodeRow in this.RecodeTable.Rows)
                        {
                            ...
                            {
                                if (recodedValue.ToUpper().Equals(configYesValue))
                                {
                                    if (string.IsNullOrEmpty(ElseValue))
                                    {
                                ...
                                else if (recodedValue.ToUpper().Equals(configNoValue))
                                {
