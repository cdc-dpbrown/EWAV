...
namespace Ewav.Web.EpiDashboard
{
    ...
    public class DataFilters : IEnumerable<FilterCondition>
    {
        ...
        private StringLiterals sl = new StringLiterals();
        ...
        public DataFilters(DashboardHelper dashboardHelper)
        {
            ...
            operandTypes.Add(SharedStrings.FRIENDLY_OPERATOR_EQUAL_TO, sl.EQUAL);
            operandTypes.Add(SharedStrings.FRIENDLY_OPERATOR_GREATER_THAN, sl.GREATER_THAN);
            operandTypes.Add(SharedStrings.FRIENDLY_OPERATOR_GREATER_THAN_OR_EQUAL, sl.GREATER_THAN_OR_EQUAL);
            operandTypes.Add(SharedStrings.FRIENDLY_OPERATOR_LESS_THAN, sl.LESS_THAN);
            operandTypes.Add(SharedStrings.FRIENDLY_OPERATOR_LESS_THAN_OR_EQUAL, sl.LESS_THAN_OR_EQUAL);
            ...
            for (int i = 0; i < rows.Length; i++)
            {
                ...
                if (condition.IsEnabled)
                {
                    if (!addedFirstCondition)
                    {
                        ...
                        prevJoin = row[COLUMN_JOIN].ToString();
                    }
                    else
                    {
                        string join = string.Empty;
                        //if (prevJoin.Length > 0)
                        //{
                        join = row.Table.Rows[i - 1][1].ToString(); //prevJoin;
                        //    prevJoin = string.Empty;
                        //}
                        //else
                        //{
                        //    row[COLUMN_JOIN].ToString();
                        //}
                        if (string.IsNullOrEmpty(join))
                        {
                        ...
                        filterCriteria.Append(sl.SPACE);
                        filterCriteria.Append(condition.Condition);
                    }
                    ...
                    if (useCondition && !condition.IsBetween && condition.IsEnabled && !string.IsNullOrEmpty(condition.Operand) && !condition.Operand.Contains("null"))
                    {
                        if (!addedFirstCondition)
                        {
                            filterCriteria.Append(" " +
                                sl.LEFT_SQUARE_BRACKET +
                                trimmedColumnName +
                                sl.RIGHT_SQUARE_BRACKET +
                                sl.SPACE +
                                condition.Operand +
                                sl.SPACE +
                                "@" + trimmedColumnName);
